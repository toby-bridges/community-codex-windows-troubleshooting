...
The runner covers C001-C019 from the research guide. It uses live GitHub issue checks, local read-only diagnostics, and `%TEMP%` fixtures for reversible reproductions. Use `-KeepArtifacts` only when the user explicitly wants to inspect temporary fixtures.
