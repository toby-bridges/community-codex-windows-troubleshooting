...
- X community case, 2026-06-06: `winget install codex -s msstore` failed before package resolution with a localized "`winget` is not recognized as an internal or external command" error. Evidence level C for the Codex install flow; the WinGet failure class itself is covered by Microsoft and microsoft/winget-cli troubleshooting docs.
